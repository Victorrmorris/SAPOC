...
for idx, row in patients.iterrows():
    with cols[idx % 3]:
        ...
        if col_a.button("Resolve", key=f"ack_{row.patient_id}"):
            st.session_state.alerts.loc[(st.session_state.alerts.patient_id == row.patient_id) & (st.session_state.alerts.status == "Active"), "status"] = "Resolve"
        if col_b.button("Dismiss", key=f"dismiss_{row.patient_id}"):
